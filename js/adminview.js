...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function syncWithSupabase() {
        ...
        try {
            ...
            const queries = tables.map(table => table === 'templates' ? supabaseClient.from(table).select('*, projects(name)') : supabaseClient.from(table).select('*'));
  ...
  `;
    ...
    async function initializeApp() {
        ...
        dom.editorForm.addEventListener('submit', async (e) => {
            ...
            const { error } = id ? await supabaseClient.from('templates').update(templateData).eq('id', id) : await supabaseClient.from('templates').insert(templateData);
    ...
    function setupDataManager() {
        ...
        document.getElementById('save-snippets-btn').addEventListener('click', async () => {
            ...
            const { error } = await supabaseClient.from('settings').upsert(snippets, { onConflict: 'key' });
        ...
        document.getElementById('add-project-btn').addEventListener('click', async () => {
            ...
            if (name) { await supabaseClient.from('projects').insert({ name, color: '#6B7280' }); await syncWithSupabase(); document.getElementById('new-project-name').value = ''; }
        });
        ...
        document.getElementById('load-default-data-btn').addEventListener('click', async () => {
            ...
            await supabaseClient.from('settings').upsert(defaultData.settings, { onConflict: 'key' });
            await supabaseClient.from('projects').upsert(defaultData.projects, { onConflict: 'name' });
            await supabaseClient.from('signatures').upsert(defaultData.signatures, { onConflict: 'name' });
            await supabaseClient.from('placeholders').upsert(defaultData.placeholders, { onConflict: 'key' });
            const { data: projects } = await supabaseClient.from('projects').select('id, name');
            ...
            for (const tpl of defaultData.templates) {
                ...
                await supabaseClient.from('templates').insert(tpl);
            }
        ...
        document.getElementById('delete-all-my-data-btn').addEventListener('click', async () => {
            ...
            for (const table of tables) { const { data } = await supabaseClient.from(table).select('id'); if (data?.length > 0) await supabaseClient.from(table).delete().in('id', data.map(d => d.id)); }
    ...
    async function handleDataImport(storeName, data, mode) {
        if (mode === 'replace') { const { data: ids } = await supabaseClient.from(storeName).select('id'); if (ids?.length > 0) await supabaseClient.from(storeName).delete().in('id', ids.map(i => i.id)); }
        const { error } = await supabaseClient.from(storeName).upsert(data);
    ...
    function renderProjectsTable() {
        ...
        container.onclick = async (e) => { if (e.target.matches('button') && confirm('Are you sure?')) { await supabaseClient.from('projects').delete().eq('id', e.target.dataset.id); await syncWithSupabase(); } };
        container.onchange = async (e) => { if (e.target.matches('.project-color-picker')) { await supabaseClient.from('projects').update({ color: e.target.value }).eq('id', e.target.dataset.id); await syncWithSupabase(); } };
    }
    function renderTemplatesTable() {
        ...
        container.onclick = async (e) => { if (e.target.matches('button') && confirm('Are you sure?')) { await supabaseClient.from('templates').delete().eq('id', e.target.dataset.id); await syncWithSupabase(); } };
    }
    function renderSignaturesTable() {
        ...
        container.onclick = async (e) => {
            ...
            if (action === 'delete' && confirm('Are you sure?')) await supabaseClient.from('signatures').delete().eq('id', id);
            if (action === 'setDefault') { await supabaseClient.from('signatures').update({ isDefault: false }).neq('id', -1); await supabaseClient.from('signatures').update({ isDefault: true }).eq('id', id); }
            await syncWithSupabase();
        };
    }
    ...
    async function handleSavePlaceholder() {
        ...
        const { error } = await supabaseClient.from('placeholders').upsert(data, { onConflict: 'key' });
    ...
    async function handleCsvImport(file) {
        ...
        try {
            ...
            const { data: existingChildren } = await supabaseClient.from('children').select('id');

            if (existingSuppliers?.length > 0) {
                await supabaseClient.from('suppliers').delete().in('id', existingSuppliers.map(s => s.id));
            }
    ...
    const openWdnSupplierEmailModal = (template) => new Promise(r => {
        ...
        if (titleEl) titleEl.textContent = `ðŸ“§ ${template.name}`;
    ...
    function setupHelpAndTutorial() {
        dom.helpContent.innerHTML = `<h2>Welcome!</h2><p>This tool helps you draft emails quickly. All your data is stored securely in the cloud.</p><h3>Key Features</h3><ul><li><strong>Centralized Data:</strong> Data is stored on supabaseClient, accessible from anywhere.</li><li><strong>Cloud Data Management:</strong> Your templates, projects, and signatures are synced.</li><li><strong>Advanced Editor:</strong> Use placeholders and optional paragraphs (scenarios) to create flexible templates.</li></ul>`;
    ...
    function setupInteractiveEditor(content) {
        ...
        dom.componentSource.innerHTML = [{ type: 'placeholder-entry', label: 'Placeholder (Text)', icon: 'T' }, { type: 'placeholder-droplist', label: 'Placeholder (List)', icon: 'â–¼' }, { type: 'placeholder-date', label: 'Placeholder (Date)', icon: 'ðŸ“…' }, { type: 'scenario', label: 'Optional Paragraph', icon: 'Â¶' },].map(c => `<div class="component-item" data-type="${c.type}"><span class="font-bold text-lg text-[rgb(var(--color-accent-primary))]">${c.icon}</span><span>${c.label}</span></div>`).join('');
...
async function previewManualAssignment() {
    ...
    try {
        ...
        previewContent.innerHTML = `
                    <div class="space-y-2">
                        <div><strong>ðŸ‘¤ Agent:</strong> ${member.name}</div>
                        <div><strong>ðŸ¢ Account:</strong> ${accountId}</div>
                        <div><strong>ðŸ“… Dates:</strong></div>
        ...
        if (assignmentType === 'daily') {
            ...
            previewContent.innerHTML = `
                        <div class="bg-blue-50 border border-blue-200 rounded p-2 mb-2">
                            ...
                            ðŸ‘¤ <strong>Agent:</strong> ${nextAssignment.agentName}<br>
                            ðŸ¢ <strong>Account:</strong> ${nextAssignment.account_export_name}<br>
                            ðŸ“… <strong>Date:</strong> ${new Date().toLocaleDateString()}<br>
                            ðŸ”„ <strong>Rotation Order:</strong> #${nextAssignment.rotation_order}
                        </div>
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded p-2 mb-2">
                            ...
                            ${weeklyAssignments.map((assignment) => `
                                <div class="text-sm mt-1">
                                    ðŸ“… <strong>${assignment.date}:</strong> ${assignment.agentName} â†’ ${assignment.account_export_name}
                                </div>
                            `).join('')}
                        </div>
                        <div class="text-xs text-gray-600 mt-1">
...
async function getNextAutoAssignment(dayOffset = 0) {
    try {
        ...
        const { data: agentList, error: agentError } = await supabaseClient
        ...
        const { data: accountList, error: accountError } = await supabaseClient
        ...
        const { data: rotationState, error: stateError } = await supabaseClient
            .from('rotation_state')
            .select('*')
            .eq('id', 1)
            .single();
...
async function addToRotationSchedule() {
    ...
    try {
        ...
        const { data: maxOrder, error: maxError } = await supabaseClient
            .from('schedule_rotation_list')
